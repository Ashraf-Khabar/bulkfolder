...
class UnzipperPage(ctk.CTkFrame):
    def __init__(self, master, on_choose_folder, on_refresh, on_extract_selected):
        ...
        self.switch_delete = ctk.CTkSwitch(controls, text="Delete archives after extraction", progress_color=DR_PURPLE)
    ...
    def _show_preview(self, path: Path) -> None:
        ...
        try:
            ...
            info_text = f"🗜️ Archive: {path.name}\n\n📦 Size: {size_mb:.2f} MB\n🕒 Modified: {mod_time}"
    ...
    def render_archives(self, archives: list[Path]) -> None:
        ...
        self._update_extract_btn_state()
        self.preview_info_lbl.configure(text="Select an archive to view info", text_color=DR_MUTED)

        if not archives:
            ctk.CTkLabel(self.scroll, text="No ZIP or TAR archives found.", text_color=DR_MUTED).pack(pady=40)
            return
        ...
        for path in archives:
            ...
            chk = ctk.CTkCheckBox(row, text="", variable=var, width=20, fg_color=DR_PURPLE, hover_color=DR_ACCENT, command=self._update_extract_btn_state)
