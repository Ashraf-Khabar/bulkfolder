...
class EmptyFoldersPage(ctk.CTkFrame):
    def __init__(self, master, on_choose_folder, on_refresh, on_delete_selected):
        ...
        ctk.CTkLabel(header, text="Empty Folders Cleaner", font=ctk.CTkFont(size=20, weight="bold"), text_color=DR_TEXT).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(header, text="Find and safely remove completely empty directories", font=ctk.CTkFont(size=12), text_color=DR_MUTED).grid(row=1, column=0, sticky="w", pady=(6, 0))
        ...
        self.btn_refresh = ctk.CTkButton(
            controls, text="Scan for Empty Folders", command=self._on_refresh,
            fg_color=DR_ACCENT, hover_color=DR_ACCENT_HOVER, text_color=DR_TEXT, width=180
        )
        ...
        self.btn_del_selected = ctk.CTkButton(
            controls, text="Delete Selected", command=self._handle_delete_selected,
            fg_color="#ff5555", hover_color="#ff7777", text_color="#ffffff", width=140, state="disabled"
        )
    ...
    def _handle_delete_selected(self):
        ...
        if paths_to_delete:
            self._on_delete_selected(paths_to_delete)

    def render_folders(self, folders: list[Path]):
        ...
        if not folders:
            ctk.CTkLabel(self.scroll, text="No empty folders found in this directory.", text_color=DR_MUTED).pack(pady=40)
            return
        ...
        for path in folders:
            ...
            ctk.CTkLabel(info_frame, text=f"📁 {path.name}", font=ctk.CTkFont(weight="bold"), text_color=DR_TEXT).pack(anchor="w")
        ...
        # Update button state immediately since items are checked by default
