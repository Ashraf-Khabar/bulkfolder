...
class PresetsPage(ctk.CTkFrame):
    def __init__(self, master, on_refresh, on_apply, on_save, on_delete):
        ...
        self.btn_refresh = ctk.CTkButton(
            row, text="Refresh", command=self._on_refresh,
            fg_color=DR_SURFACE, hover_color=DR_BORDER, text_color=DR_TEXT, 
            border_color=DR_BORDER, border_width=1, width=110, height=32,
        )
        ...
        self.btn_apply = ctk.CTkButton(
            row2, text="Apply", command=self._on_apply,
            fg_color=DR_ACCENT, hover_color=DR_ACCENT_HOVER, text_color=DR_TEXT, width=120, height=32,
        )
        self.btn_apply.pack(side="left", padx=(0, 8))

        self.btn_delete = ctk.CTkButton(
            row2, text="Delete", command=self._on_delete,
            fg_color=DR_SURFACE, hover_color=DR_BORDER, text_color=DR_TEXT, 
            border_color=DR_BORDER, border_width=1, width=120, height=32,
        )
        self.btn_delete.pack(side="left", padx=(0, 8))

        self.btn_save = ctk.CTkButton(
            row2, text="Save current", command=self._on_save,
            fg_color=DR_PURPLE, hover_color=DR_PURPLE, text_color=DR_TEXT, width=140, height=32,
        )
        ...
        self._text.tag_configure("KEY", foreground="#bd93f9") # Highlighting
    ...
    def _render_details(self) -> None:
        """Update the info display with selected preset data."""
