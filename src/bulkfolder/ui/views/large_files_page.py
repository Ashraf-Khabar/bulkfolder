...
class LargeFilesPage(ctk.CTkFrame):
    def __init__(self, master, on_choose_folder, on_refresh):
        # Initialisation sans on_delete_selected car la page est en lecture seule
        ...
        ctk.CTkLabel(header, text="Large Files (View Only)", font=ctk.CTkFont(size=20, weight="bold"), text_color=DR_TEXT).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(header, text="Analyze the heaviest files in your folders", font=ctk.CTkFont(size=12), text_color=DR_MUTED).grid(row=1, column=0, sticky="w", pady=(6, 0))
        ...
        self.btn_refresh = ctk.CTkButton(controls, text="Scan & Analyze", command=self._handle_refresh, fg_color=DR_ACCENT, hover_color=DR_ACCENT_HOVER, text_color=DR_TEXT, width=140)
        ...
        self.preview_info_lbl = ctk.CTkLabel(self.preview_panel, text="Select a file to view details", text_color=DR_MUTED, justify="left", wraplength=240)
    ...
    def _show_preview(self, path: Path):
        try:
            ...
            self.preview_info_lbl.configure(text="Analysis not available", text_color=DR_MUTED)

    def render_files(self, files: list[tuple[Path, int]]):
        for widget in self.scroll.winfo_children(): widget.destroy()
        self.preview_info_lbl.configure(text="Select a file to view details", text_color=DR_MUTED)

        if not files:
            ctk.CTkLabel(self.scroll, text="No files found.", text_color=DR_MUTED).pack(pady=40)
            return
