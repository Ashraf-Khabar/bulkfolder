...
class LargeFilesPage(ctk.CTkFrame):
    def __init__(self, master, on_choose_folder, on_refresh, on_delete_selected):
        ...
        self.btn_refresh = ctk.CTkButton(
            controls, text="Scan & Filter", command=self._handle_refresh,
            fg_color=DR_ACCENT, hover_color=DR_ACCENT_HOVER, text_color=DR_TEXT, width=140
        )
        self.btn_refresh.pack(side="left")

        # Select All & Delete Selected Buttons
        # CORRECTION ICI: fg_color au lieu de progress_color
    ...
    def _handle_refresh(self):
        try:
            min_mb = float(self.min_mb_var.get())
        except ValueError:
            min_mb = 0.0
    ...
    def render_files(self, files: list[tuple[Path, int]]):
        ...
        if not files:
            ctk.CTkLabel(self.scroll, text="No files match these criteria in this folder.", text_color=DR_MUTED).pack(pady=40)
            return
        ...
        for path, size in files:
            ...
            ctk.CTkLabel(info_frame, text=f"{path.name}  —  {size_mb:.1f} MB", font=ctk.CTkFont(weight="bold"), text_color=DR_TEXT).pack(anchor="w")
